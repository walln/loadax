...
def test_large_batch_size():
    ...
    batcher = Batcher(lambda x: x)
    loader = DataLoader(batcher).batch_size(10).build(dataset)
...
def test_small_batch_size():
    dataset = InMemoryDataset([1, 2, 3, 4, 5])
    batcher = Batcher(lambda x: x)
    loader = DataLoader(batcher).batch_size(1).build(dataset)
...
def test_dataloader_custom_batch_function():
    ...
    loader = DataLoader(batcher).batch_size(3).build(dataset)
