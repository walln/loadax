...
from loadax import Batcher, InMemoryDataset
...
# The data shard axis is used to determine how to construct the global batch from the
# local batches. This is optional, but the most convenient way to do FSDP training.
# To understand why, see the documentation for the DistributedShardingStrategy
...
def distributed_dataloader_with_parameter_sharding():
    """Simple training loop with data parallelism and parameter sharding."""
    ...
    for local_batch in dataloader:
        # You still have total control over array placement, so you can decide how to
        # parallelize your intra-node batch across the local devices, when to
        # synchronize across nodes, etc.
        # This is a really powerful set of primitives and you do not need to use pjit,
        # this is just an example of how you can leverage Jax's powerful parallelization
        # primitives in combination with Loadax to achieve distributed training.
...
distributed_dataloader_with_parameter_sharding()
